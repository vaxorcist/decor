...
# version 2.3
# v2.3 (Session 88, Storage Locations feature Session E — see
...
#         components = components.where(storage_location_id: params[:storage_location_id])
#       end
#     end
...
#        filter dropdown (ComponentsHelper#component_filter_storage_locations_options)
...
#        for any X — letting a logged-in user enumerate which components
...
class ComponentsController < ApplicationController
  ...
  def index
    ...
    # storage_location_id that actually belongs to Current.owner. Mirrors
    # ComputersController#index v1.25 exactly — see the header comment above
    # (Session E) for the full rationale on both guards.
    if logged_in? && params[:storage_location_id].present?
      if Current.owner.storage_locations.exists?(id: params[:storage_location_id])
        components = components.where(storage_location_id: params[:storage_location_id])
      end
