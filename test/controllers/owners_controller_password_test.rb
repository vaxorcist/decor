# decor/test/controllers/owners_controller_password_test.rb - version 1.5
# Refactored to use centralized AuthenticationHelper constants
# Removed local login_as method - now inherited from test/support/authentication_helper.rb
...
class OwnersControllerPasswordTest < ActionDispatch::IntegrationTest
  ...
  test "should change password with correct current password" do
    login_as(@owner)
    patch owner_path(@owner), params: { owner: { current_password: TEST_PASSWORD_ALICE, password: "newpass456789", password_confirmation: "newpass456789" } }
    assert_redirected_to owner_path(@owner)
    @owner.reload
    assert @owner.authenticate("newpass456789")
  end

  test "should not change password with incorrect current password" do
    login_as(@owner)
    patch owner_path(@owner), params: { owner: { current_password: "wrong", password: "newpass456789", password_confirmation: "newpass456789" } }
  ...
  test "should not change password when confirmation does not match" do
    login_as(@owner)
    patch owner_path(@owner), params: { owner: { current_password: TEST_PASSWORD_ALICE, password: "newpass456789", password_confirmation: "different" } }
    assert_response :unprocessable_entity
    @owner.reload
    assert @owner.authenticate(TEST_PASSWORD_ALICE)
  end
  ...
  test "should not accept new password without current password" do
    login_as(@owner)
    patch owner_path(@owner), params: { owner: { current_password: "", password: "newpass456789", password_confirmation: "newpass456789" } }
    assert_response :unprocessable_entity
  end

  test "should change password and update profile fields simultaneously" do
    login_as(@owner)
    patch owner_path(@owner), params: { owner: { real_name: "New Name", website: "https://newsite.com", current_password: TEST_PASSWORD_ALICE, password: "newpass456789", password_confirmation: "newpass456789" } }
    assert_redirected_to owner_path(@owner)
    @owner.reload
